...
def postcode_to_sector(postcode):
    ''' A function to convert a postcode to a sector by extracting the outward
        code and the first digit of the inward code.
        Arguments:
        ...
        'SW1A 1'
    '''
    # Cleaning the postcode by removing leading/trailing white spaces and
    # converting to uppercase
    postcode = re.sub(r'\s+', ' ', postcode.strip()).upper()
 
    # Validating the postcode format using a regex
    match = re.match(
        r'^([A-Z]{1,2}\d[A-Z\d]?) ?(\d[A-Z]{2})$', postcode
    )
...
def postcode_to_district(postcode):
    ''' A function to convert a postcode to a district by extracting the
        outward code.
...
def normalise_sector(sector):
    ''' A function to normalise the postcode sector by removing leading/
    trailing white spaces and converting to uppercase.
        ...
        'SW1A 1'
    '''
    sector = re.sub(r'\s+', ' ', sector.strip()).upper()
...
def normalise_district(district):
    ''' A function to normalise the postcode district by removing leading/
    trailing white spaces and converting to uppercase.
    ...
    match = re.match(r'^([A-Z]{1,2}\d{1,2})$', district)
